...
1. **RedNews install runs without the launcher.** This copy deliberately ships no `scripts/` folder (no binary, no hooks), so never try to run `scripts/impeccable`. Instead read PRODUCT.md and DESIGN.md (project root of the frontend, e.g. `app/`) directly, without inventing missing context. Wherever a reference says to run a `scripts/impeccable <verb>` command, do the equivalent by reading and reasoning instead; skip the steps that only the binary can do (detector scans, pin, hooks, doctor, live browser mode, generate) and say so briefly if the user asked for one of them.
...
**Not available in this install:** `pin`, `hooks`, `doctor`, `live` and `generate` depend on the omitted binary. If asked, say they are disabled in this project and offer the closest manual alternative (e.g. critique/polish instead of the detector).